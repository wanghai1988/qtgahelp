...
(3) 快速成图：实现活动断层、形变场、主应变率、块体旋转率、震源机制解和地震目录数据的快速绘制，提供制图过程必需的地图符号，提供比例尺、图例、文字等必需的布局元素，并可将图片按指定格式输出。
(4) 剖面分析：可以绘制地形、InSAR等栅格数据的剖面图，以及对GNSS速率进行剖面分析。
(5) 数据编辑：提供常见数据的提取、格式转换功能。
(6) 地震周期形变的正演：可以构建矩形元或三角元的断层几何模型，以及格林函数的计算，正演同震、震间变形场。
